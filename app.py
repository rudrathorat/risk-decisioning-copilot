...
SAVINGS_OPTIONS = [
    ("No savings / Not disclosed", "A61"),
    ("< 100 DM  (Low)", "A62"),
    ("100–500 DM  (Moderate)", "A63"),
    ("500–1,000 DM  (Good)", "A64"),
    (">= 1,000 DM  (Strong)", "A65"),
]
EMPLOYMENT_OPTIONS = [
    ("Unemployed", "A71"),
...
def render_risk_bar(risk_score: float) -> None:
    ...
    st.markdown(
        ...
        f'</div><p style="margin: 0 0 12px 0; font-size: 0.85rem; color: #64748b;">Default Risk Indicator: Low → High</p>',
...
st.caption("Enter an applicant profile to generate an analyst-style credit report — including risk score, key risk drivers, suggested conditions, and what-if scenario analysis.")

with st.spinner("Building credit risk pipeline — feature engineering, model training... (one-time)"):
...
with st.container():
    ...
    st.caption("Inputs are scored against a model trained on the German Credit Dataset (1,000 applicants, WOE/IV feature engineering, 0.78 ROC-AUC).")
...
if st.button("Run Credit Analysis", type="primary"):
    ...
    st.divider()
    st.caption(f"Credit report for a **{currency} {loan_amount:,.0f}** loan over **{duration} months** — generated by the AI Underwriting Copilot.")

    # 📊 Risk Assessment — Decision summary + visual risk indicator
    st.markdown("### 📊 Credit Risk Assessment")
    m1, m2, m3 = st.columns(3)
    with m1:
        st.metric("Probability of Default (PD)", f"{report.risk_score:.1%}")
    with m2:
        st.metric("Risk Band", report.risk_band)
    with m3:
        st.metric("Credit Decision", report.recommended_decision)
    render_risk_bar(float(report.risk_score))

    st.divider()

    # ⚠️ Key Risk Drivers
    st.markdown("### ⚠️ Key Risk Drivers")  # matches CopilotReport.key_risk_drivers
    if report.key_risk_drivers:
        for d in report.key_risk_drivers[:8]:
            narrative = d.get("narrative", d.get("description", ""))
            st.markdown(f"- {narrative}")
    else:
        st.caption("No significant risk factors identified for this profile.")
    ...
    if report.suggested_conditions:
        ...
        st.caption("No conditions required for this risk profile.")
    ...
    if report.what_if_results:
        ...
        st.caption("No scenario results available.")

    st.divider()
    with st.expander("AI Interpretation (Full Narrative)"):
